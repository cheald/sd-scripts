...
def add_training_arguments(parser: argparse.ArgumentParser, support_dreambooth: bool):
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="If >0, dynamically center latents during training for better subject transfer. 5e-2 is a good starting point.",
    )
    parser.add_argument(
        "--latent_corruption",
        type=float,
        default=None,
        help="Corrupt latents by blending in random noise. Can improve regularization.",
    )
    parser.add_argument(
        "--multires_discount_lr",
        type=float,
        default=None,
...
def get_noise_noisy_latents_and_timesteps(args, noise_scheduler, latents, network):
    ...
    # discount, sigmoid(discount), rrelu(discount) both work well
    # discount^2 and discount^0.5 has problems with vanishing gradients
    # rrelu converges more slowly than the others, but seems to preserve
    ...
    if args.latent_corruption is not None:
        # Corrupt the latents by reducing the value of `args.latent_corruption` percentage of pixels by a random value.
        corruption = torch.rand_like(latents, device=latents.device) #* (1-noise_scheduler.alphas_cumprod.to(latents.device)[timesteps]).view(-1, 1, 1, 1)
        mask = torch.rand_like(latents, device=latents.device) * (corruption < args.latent_corruption).int() + (corruption > args.latent_corruption).int()
        latents = latents * mask
        # print("div", latents.mean(dim=(1,2,3)))
        # latents = latents / ((1+latents.mean(dim=(1,2,3))) * (1+div)).view(div.shape[0], -1, 1, 1)

    if args.latent_centering_lr is not None:
        softsign = torch.nn.Softsign()
        latents = latents - (latents.mean(dim=(2,3)) - softsign(network.latent_mean_offset)).view(-1, 4, 1, 1) * network.latent_mean_scale
